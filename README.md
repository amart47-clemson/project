...
- **Analyze area** — Run tree crown detection (when backend is up), biomass/carbon estimation, and satellite-derived metrics for the drawn area(s) or the quick circle.
- **Results** — Tree crown detection count (DeepForest), satellite-derived (NDVI, vegetation cover), summary (tree count, biomass, carbon, CO₂ equivalent, volume), species breakdown, and tree-level table.
