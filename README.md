...
- **GEDI (NASA ISS lidar)** — When the backend is running and the area is within 51.6°N–51.6°S, the app fetches **GEDI L4A** data: laser pulse measurements of canopy height (RH98) and aboveground biomass density (AGBD, Mg/ha). These are among the most accurate remote biomass estimates available. Requires [NASA Earthdata login](https://urs.earthdata.nasa.gov) (set `~/.netrc` or env vars). **→ [Step-by-step setup: Connect GEDI & NASA Earthdata](docs/GEDI_AND_EARTHDATA_SETUP.md)**
